...
# at any given time.
VERSION = "1.3.11"  # Defines the version number, for use in internal tracking.
...
async def on_member_ban(guild, user):  # This event executes when the bot hears a ban.
    ...
    if entry.user.id == bot.user.id and "Your account is too new" in entry.reason:  # Functions in this block
...
async def on_member_join(member):  # This event executes when the bot hears a join.
    ...
    threshold = re.search(r"(^\d{1,2})(m$|h$|d$)", account_age_threshold)
    threshold_number = int(threshold.group(1))
    threshold_letter = threshold.group(2)
    if threshold_letter == "m":
        threshold = threshold_number * 60
    elif threshold_letter == "h":
        threshold = threshold_number * 3600
    elif threshold_letter == "d":
        threshold = threshold_number * 86400
    creation_time = datetime.strptime(str(member.created_at).partition(".")[0], "%Y-%m-%d %H:%M:%S")
    age = int(str((datetime.strptime(str(datetime.now(timezone.utc)).partition(".")[0],
                                     "%Y-%m-%d %H:%M:%S") - creation_time).total_seconds()).partition(".")[0])
    if age < threshold:
        try:
            ...
            pass
        embed = discord.Embed(color=discord.Color(0xf02a07), description="""Your account is too new to join this \
...
#     channel = bot.get_channel(348897378062827520)
#     guild = member.guild
#     if guild.id != channel.guild.id:
#         return
#     with open("auto_punish_info.json", "r") as auto_punish_info:  # Opens the auto_punish_info json, for use in
...
# this Discord! Please visit [our ban appeal page](https://unban.aetherhunts.net/) to appeal.""",
#                               color=discord.Color(0xf02a07))
...
#         except HTTPException:
#             pass
#         await member.guild.ban(member, reason=f"""Your account is too new to join \
# this Discord! Please visit [our ban appeal page](https://unban.aetherhunts.net/) to appeal.""")  # Bans # the member.
#         return
# I am so fucking tired of fixing this. The uncommented code is from Rewrite. I guess we'll see if it works.
...
async def milestone_listener(before, after):
    ...
    if licensed_hunter_role_count == 40000 and\
        ...
        await channel.send(f"""{after.mention} is our 40.000th Licensed Hunter!
...
**Stage 3** | 2-12 Hours after Stage 2""", inline=True)
...
@bot.command(aliases=["info"], name="unlink")
async def unused(ctx):
    await ctx.send(f"""Sorry, but this command is temporarily unavailable!
Following an emergency update to `{PREFIX}link`, the `{PREFIX}info` and `{PREFIX}unlink` commands were not updated. \
Please be patient while the updates are being worked on, and expect to see them by (or at some point around) \
Endwalker!
If you are attempting to unlink your character and it is considered a high-priority task to you, please contact \
the developer by opening a support ticket and saying that you would like to have your character unlinked.
